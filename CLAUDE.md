...
`headroom` answers two questions and acts on the answers: **which of this machine's Claude Code accounts has quota left** (`headroom accounts`, the board: live limit bars, refreshing while it is open, enter records the chosen account) and **which session do I resume, on which account** (`headroom resume`: every session on the machine in one picker — transcripts are machine-global — resumed in its own project dir on the account that last drove it). Acting on either answer goes through `headroom launch`, which owns launch routing: it validates the account and execs `claude` with an environment built from that decision alone. `headroom check` verifies the reverse-engineered vendor facts all of it rests on. The mental model and those vendor contracts live in [DESIGN.md](DESIGN.md) — read it before touching the parsers, `check`, the session store, or anything Keychain-related.
