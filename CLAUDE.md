...
The session surface is a second, network-free pipeline: `internal/sessions` walks `~/.claude/projects` (tail-parses each transcript for title/preview/cwd/model, attributes it to an account via per-account `history.jsonl` + the live-session registry + explicit re-homes from the state file), and `app/resume.go` renders it on `/dev/tty` (alt screen). Enter chdirs into the verified project dir and **execs claude in-process** through `launch.Target` — no decision line, no wrapper recombination; the optional `--cd-file` carries the entered dir out for the shell's own post-session cd (empty = no launch committed). Owner semantics, the cd-verification rule, liveness gating, the exec ordering and the mutation exception are specified in DESIGN.md § The session surface.

The launch surface (`internal/launch` + `app/launch.go`) turns a validated account into a running `claude`: `resolve` prints name/dir/kind for shell preflight, `launch` revalidates, optionally records `.current` (`--remember`), and execs with the environment `launch.Target` constructs. `auth`'s probe subprocesses build their environments through the same constructor.
